    ...
    <div className="min-h-screen bg-[#020817] text-white overflow-x-hidden" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <main id="main-content" className="relative" style={{ zIndex: 1 }}>
            ...
            <div className="card-glass rounded-2xl sm:rounded-3xl overflow-hidden">
              ...
              <div className="border-t border-white/10 p-4 sm:p-6 bg-black/20">
                <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3 mb-4">
                  <button onClick={() => handleDemoButtonClick("refund")} className="tag-pill text-xs sm:text-sm px-4 py-2 rounded-full hover:bg-indigo-500/20 transition-all hover:scale-105">Refund Policy</button>
                  <button onClick={() => handleDemoButtonClick("pricing")} className="tag-pill text-xs sm:text-sm px-4 py-2 rounded-full hover:bg-indigo-500/20 transition-all hover:scale-105">Pricing</button>
                  <button onClick={() => handleDemoButtonClick("api")} className="tag-pill text-xs sm:text-sm px-4 py-2 rounded-full hover:bg-indigo-500/20 transition-all hover:scale-105">API Access</button>
                  <button onClick={() => handleDemoButtonClick("cancel")} className="tag-pill text-xs sm:text-sm px-4 py-2 rounded-full hover:bg-indigo-500/20 transition-all hover:scale-105">Cancel Subscription</button>
                </div>
                <div className="flex justify-center">
                  <button onClick={clearChat} className="text-white/30 hover:text-white/60 text-xs flex items-center gap-1 transition-colors">
        ...
        <section className="py-12 sm:py-16 px-4 sm:px-6 border-y border-white/5">
          <div className="max-w-5xl mx-auto">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6">
              {[
                { value: "10,000+", label: "Daily Conversations", icon: MessageSquare, color: "#818cf8" },
        ...
        <section id="features" className="py-16 sm:py-24 px-4 sm:px-6 border-t border-white/5">
          <div className="max-w-7xl mx-auto">
            <div className="text-center mb-10 sm:mb-16">
              ...
              <p className="text-white/40 max-w-xl mx-auto text-base sm:text-lg px-2">From knowledge base management to embeddable widgets — the full stack covered.</p>
            </div>
        ...
        <section id="how-it-works" className="py-16 sm:py-24 px-4 sm:px-6 border-t border-white/5 bg-gradient-to-b from-transparent to-indigo-950/10">
          <div className="max-w-7xl mx-auto">
            <div className="text-center mb-10 sm:mb-16">
              ...
              <p className="text-white/40 max-w-2xl mx-auto text-base sm:text-lg px-2">Get from zero to AI-powered support in three easy steps — no engineering team required.</p>
            </div>
        ...
        <section id="testimonials" className="py-16 sm:py-24 px-4 sm:px-6 border-t border-white/5">
          <div className="max-w-7xl mx-auto">
            <div className="text-center mb-10 sm:mb-16">
              ...
              <p className="text-white/40 max-w-2xl mx-auto text-base sm:text-lg px-2">Teams use SmartSupport AI to reduce support workload and improve customer experience.</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
        ...
        <section id="pricing" className="py-16 sm:py-24 px-4 sm:px-6 border-t border-white/5">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
              {plans.map((plan) => (
                <div key={plan.name} className={`rounded-2xl p-5 sm:p-6 flex flex-col transition-all duration-300 hover:-translate-y-1 ${plan.highlighted ? "plan-popular glow" : "card-glass"}`}>
                  {plan.badge && <div className="inline-flex items-center gap-1.5 tag-pill text-xs font-semibold px-3 py-1 rounded-full self-start mb-3"><Sparkles className="w-3 h-3" /> {plan.badge}</div>}
                  {plan.name === "Free" && !plan.badge && <div className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1 rounded-full self-start mb-3" style={{ background: "rgba(16,185,129,0.15)", color: "#34d399", border: "1px solid rgba(16,185,129,0.3)" }}>✦ Always free</div>}
                  <h3 className="font-display text-xl sm:text-2xl font-bold mb-1 text-white">{plan.name}</h3>
                  <div className="flex items-baseline gap-1 mb-2"><span className="font-display text-3xl sm:text-4xl font-extrabold text-white">{plan.price}</span><span className="text-white/30 text-sm">{plan.period}</span></div>
                  ...
                  <ul className="space-y-2 sm:space-y-3 mb-6 sm:mb-8 flex-1">
                    {plan.features.map((feat) => (
                      <li key={feat} className="flex items-start gap-2.5 text-xs sm:text-sm text-white/70">
                        <div className="w-4 h-4 sm:w-5 sm:h-5 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5" style={{ background: "rgba(99,102,241,0.2)", border: "1px solid rgba(99,102,241,0.3)" }}>
                  ...
                  <Link href="/sign-up">
                    <button className={`w-full py-2.5 sm:py-3 rounded-xl text-sm font-semibold transition-all duration-200 min-h-[44px] ${plan.highlighted ? "btn-primary text-white glow-sm" : "card-glass text-white/80 hover:text-white"}`}>
                      {plan.cta}
                    </button>
                  </Link>
                </div>
            ...
            <div className="space-y-4">
              {faqItems.map((item, idx) => (
                <div key={idx} className="card-glass rounded-xl overflow-hidden">
                  <button onClick={() => setOpenFaqIndex(openFaqIndex === idx ? null : idx)} className="w-full px-6 py-4 flex items-center justify-between text-left hover:bg-white/5 transition-colors">
                  ...
                  <div className={`faq-answer transition-all duration-300 ${openFaqIndex === idx ? 'max-h-40 opacity-100' : 'max-h-0 opacity-0'}`}>
                    <div className="px-6 pb-4 text-white/60 text-sm sm:text-base leading-relaxed">{item.answer}</div>
                  </div>
                </div>
              ))}
            </div>
        ...
        <section className="py-16 sm:py-24 px-4 sm:px-6 border-t border-white/5">
          <div className="max-w-4xl mx-auto text-center">
            <div className="rounded-2xl sm:rounded-3xl p-8 sm:p-12 md:p-14 relative overflow-hidden" style={{ background: "linear-gradient(135deg, rgba(79,70,229,0.2), rgba(124,58,237,0.2))", border: "1px solid rgba(99,102,241,0.4)" }}>
              ...
              <div className="relative">
                <h2 className="font-display text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold mb-4 gradient-text">Ready to reduce support workload by 80%?</h2>
                <p className="text-white/60 mb-8 sm:mb-10 text-base sm:text-lg max-w-xl mx-auto px-2">Join businesses already using SmartSupport AI to deliver faster, smarter customer support.</p>
      ...
      <footer className="border-t border-white/5 py-8 sm:py-10 px-4 sm:px-6" style={{ zIndex: 1, position: "relative" }}>
        <div className="max-w-7xl mx-auto flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between text-sm text-white/30">
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-lg btn-primary flex items-center justify-center flex-shrink-0"><Zap className="w-3.5 h-3.5 text-white" /></div>
